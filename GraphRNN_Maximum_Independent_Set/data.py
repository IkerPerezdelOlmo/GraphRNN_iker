...
# load the datasets a list of netwokx graphs will be returned
def Graph_load_batch(min_num_nodes = 20, max_num_nodes = 1000, name = 'random50',node_attributes = True,graph_labelsUsed=True):
    current_dir = os.getcwd()

    # Construct the path to your file dynamically
    path = os.path.join(current_dir, 'dataset/'+name+'/')

    adj_list = np.loadtxt(path+name+'_A.txt', delimiter=',').astype(int)
    adj_list = list(map(tuple, adj_list))
    graph_indicator = np.loadtxt(path+name+'_graph_indicator.txt', delimiter=',').astype(int)
    edge_labels = np.loadtxt(path+name+'_edge_labels.txt', delimiter=',').astype(int)
    data_node_label = np.loadtxt(path+name+'_node_labels.txt', delimiter=',').astype(int)
    if graph_labelsUsed:
        graph_labels = np.loadtxt(path+name+'_graph_labels.txt', delimiter=',').astype(int)
    
    if node_attributes:
        node_labels = np.loadtxt(path+name+'_node_labels.txt', delimiter=',').astype(int)
    ...
    # Create graphs for each unique graph identifier
    for graph_id in unique_graph_ids:
        # Create a new graph
        G = nx.Graph()

        # Add nodes to the graph based on the graph indicator
        nodes_in_graph = [node_id for node_id, graph_id_value in enumerate(graph_indicator, start=1) if graph_id_value == graph_id]
        G.add_nodes_from(nodes_in_graph)

        #add the nodes
        if node_attributes:
            for node in nodes_in_graph:
                G.nodes[node]['label'] = node_labels[node - 1]

        # Add edges to the graph based on the adjacency list
        edges_in_graph = [(row, col) for row, col in adj_list if row in nodes_in_graph and col in nodes_in_graph]
        G.add_edges_from(edges_in_graph)

        # Optionally, add graph labels and edge labels
        if graph_labelsUsed:
            G.graph['label'] = graph_labels[graph_id -1]  # assuming graph labels are 1-indexed
        for edge, edge_label in zip(edges_in_graph, edge_labels):
            G.edges[edge]['weight'] = edge_label
...
def get_dictionary(G, start_id):
    nodes = np.array(G.nodes()).tolist()
    dictionary = dict(nx.bfs_successors(G, start_id))
    for key in dictionary.keys():
          nodes.remove(key)
    while len(nodes) >0:
        following_id = random.choice(nodes)
        dictionary2 = dict(nx.bfs_successors(G, following_id))
        if not dictionary2:
            nodes.remove(following_id)
        else:
            for key in dictionary.keys():
                if key in dictionary2.keys():
                    dictionary2.pop(key)
                for key2 in dictionary2.keys():
                    if key in dictionary2[key2]:
                        dictionary2[key2].remove(key)
...
def bfs_seq(G, start_id):
    ...
    :return:
    '''
    dictionary = get_dictionary(G, start_id)
