...
#This is a functionto stop the execution if there is no improvement over epocos.

# si hay un momento en el que no se cumple la condición, no volver a evaluarla hasta dentro de N epochs, ya que puede haber pasado que no s de en un momento un epoch exremadament bajo, y por lo cual, durante N epochs nos vaya a dar 
class EarlyStopping:
    def __init__(self, patience=10, verbose=False, delta=0.0001, length = 30):
        self.length = length
        self.patience = patience
        self.verbose = verbose
        self.counter = 0
        self.previousLoss = [sys.float_info.max for i in range(length)]
        self.currentLoss = [sys.float_info.max for i in range(length)]
        self.early_stop = False
        self.delta = delta
        self.best_loss = sys.float_info.max
        self.waitLengthTimes = 0  # if currentLoss > previousLoss + delta, we will wait length epochs until next check, so we do not take into account bad epochs twce

    def __call__(self, val_loss):
        self.previousLoss.pop(0)
        self.previousLoss.append(self.currentLoss.pop(0))
        self.currentLoss.append(val_loss)
        currentLossValue = sum(self.currentLoss)
        if currentLossValue > (sum( self.previousLoss) + self.delta) and self.waitLengthTimes <= 0:
            self.counter += 1
            self.waitLengthTimes = self.length +1 
            if self.verbose:
                print(f'EarlyStopping counter: {self.counter} out of {self.patience}')
            if self.counter >= self.patience:
                self.early_stop = True
        elif  currentLossValue < self.best_loss + self.delta:
            self.best_loss = currentLossValue
            self.counter = 0
        self.waitLengthTimes -=1
...
def pick_connected_component(G):
    ...
    # Pick the first component starting from node 0
    for component in components:
        if 0 in component:
            return G.subgraph(component).copy()
    
    # If no component contains node 0, return an empty graph or handle as needed
    return nx.Graph()
