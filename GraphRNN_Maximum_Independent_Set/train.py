...
def train_vae_epoch(epoch, args, rnn, output, data_loader,
                    ...
                    scheduler_rnn, scheduler_output, model_index, dataset_index, early_stopping):
    rnn.train()
    output.train()
    loss_sum = 0
    for batch_idx, data in enumerate(data_loader):
        ...
        # logging
        log_value('bce_loss_'+args.fname(model_index, dataset_index), loss_bce.data.item(), epoch*args.batch_ratio+batch_idx)
        log_value('kl_loss_' +args.fname(model_index, dataset_index), loss_kl.data.item(), epoch*args.batch_ratio + batch_idx)
        log_value('z_mu_mean_'+args.fname(model_index, dataset_index), z_mu_mean, epoch*args.batch_ratio + batch_idx)
        log_value('z_mu_min_'+args.fname(model_index, dataset_index), z_mu_min, epoch*args.batch_ratio + batch_idx)
        log_value('z_mu_max_'+args.fname(model_index, dataset_index), z_mu_max, epoch*args.batch_ratio + batch_idx)
        log_value('z_sgm_mean_'+args.fname(model_index, dataset_index), z_sgm_mean, epoch*args.batch_ratio + batch_idx)
        log_value('z_sgm_min_'+args.fname(model_index, dataset_index), z_sgm_min, epoch*args.batch_ratio + batch_idx)
        log_value('z_sgm_max_'+args.fname(model_index, dataset_index), z_sgm_max, epoch*args.batch_ratio + batch_idx)
...
def test_vae_epoch(epoch, args, rnn, output, test_batch_size=16, save_histogram=False, sample_time = 1):
    ...
    #                           fname_pred=args.figure_prediction_save_path+args.fname_pred(model_index, dataset_index)+str(epoch)+'.jpg',
...
def train_mlp_epoch(epoch, args, rnn, output, data_loader,
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        if epoch % args.epochs_log==0 and batch_idx==0: # only output first batch's statistics
            ...
            early_stopping(loss.data.item())
        # logging
        log_value('loss_'+args.fname(model_index, dataset_index), loss.data.item(), epoch*args.batch_ratio+batch_idx)
...
def train_mlp_forward_epoch(epoch, args, rnn, output, data_loader, model_index, dataset_index):
    rnn.train()
    output.train()
    loss_sum = 0
    for batch_idx, data in enumerate(data_loader):
...
def train_rnn_epoch(epoch, args, rnn, output, data_loader,
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        y_len_unsorted = data['len']
        y_len_max = max(y_len_unsorted)
        # esto que estamos haciendo aquí creo que no sirve para absolutamente nada
        x_unsorted = x_unsorted[:, 0:y_len_max, :]
        y_unsorted = y_unsorted[:, 0:y_len_max, :]
        # initialize lstm hidden state according to batch size
        rnn.hidden = rnn.init_hidden(batch_size=x_unsorted.size(0))
        ...
        # sort input (ordenamos los batches de mayor a menor )
        y_len,sort_index = torch.sort(y_len_unsorted,0,descending=True)
        y_len = y_len.numpy().tolist()
        x = torch.index_select(x_unsorted,0,sort_index)
        y = torch.index_select(y_unsorted,0,sort_index)
        ...
        # aqui lo que hacemos es para cada una de las posibles dimensiones de la matriz,
        # miramos cuantas matrices hay con una dimensionalidad superior (count_temp). Esto lo guardamos en la variable output_y_len
        # en esta variable vamos a guardar count_temp veces el la dimensionalidad que tenemos.
        output_y_len = []
        ...
        if epoch % args.epochs_log==0 and batch_idx==0: # only output first batch's statistics
            print('Epoch: {}/{}, train loss: {:.6f}'.format(
                epoch, args.epochs,loss.data.item()))
            early_stopping(loss.data.item())
...
def validate_rnn_epoch(epoch, args, rnn, output, data_loader,
                       model_index, dataset_index):
    rnn.eval()
    output.eval()
    loss_sum = 0
    with torch.no_grad():  # Ensure no gradients are computed
        ...
        for batch_idx, data in enumerate(data_loader):
            ...
            # Initialize hidden state according to batch size
            rnn.hidden = rnn.init_hidden(batch_size=x_unsorted.size(0))



            # Sort input by sequence length
            y_len, sort_index = torch.sort(y_len_unsorted, 0, descending=True)
            y_len = y_len.numpy().tolist()
            x = torch.index_select(x_unsorted, 0, sort_index)
            y = torch.index_select(y_unsorted, 0, sort_index)

            # Reshape and reverse y for the output module
            y_reshape = pack_padded_sequence(y, y_len, batch_first=True).data
            idx = [i for i in range(y_reshape.size(0)-1, -1, -1)]
            idx = torch.LongTensor(idx)
            y_reshape = y_reshape.index_select(0, idx)
            ...
            h = h.index_select(0, idx)
            hidden_null = torch.zeros(args.num_layers-1, h.size(0), h.size(1)).cuda()
            ...
            # Logging
            log_value('val_loss_' + args.fname(model_index, dataset_index), loss.item(), epoch * args.batch_ratio + batch_idx)
            feature_dim = y.size(1) * y.size(2)
...
def test_rnn_epoch(epoch, args, rnn, output, test_batch_size=16):
    ...
    # el primer input es algo del tamaño de (1, pre_nodes)
    x_step = Variable(torch.ones(test_batch_size,1,args.max_prev_node)).cuda()
    for i in range(max_num_node):
        h = rnn(x_step)
...
def train_rnn_forward_epoch(epoch, args, rnn, output, data_loader,model_index, dataset_index):
    rnn.train()
    output.train()
    loss_sum = 0
    for batch_idx, data in enumerate(data_loader):
...
def train(args, dataset_train, rnn, output, model_index, dataset_index, getValidationLoss, dataset_validate ):
    print("hitrain")

    # check if load existing model
    if args.load:
        fname = args.model_save_path + args.fname(model_index, dataset_index) + 'lstm_' + str(args.load_epoch) + '.dat'
        rnn.load_state_dict(torch.load(fname))
        fname = args.model_save_path + args.fname(model_index, dataset_index) + 'output_' + str(args.load_epoch) + '.dat'
    ...
    early_stopping = utils.EarlyStopping( verbose=True)
    # start main loop
    ...
    while epoch<=args.epochs:
        ...
        if 'GraphRNN_VAE' in args.note:
            ...
            train_rnn_epoch(epoch, args, rnn, output, dataset_train,
                            optimizer_rnn, optimizer_output,
                            scheduler_rnn, scheduler_output,model_index, dataset_index, early_stopping)
            if getValidationLoss:
                validate_rnn_epoch(epoch, args, rnn, output, dataset_validate,
                            model_index, dataset_index)
        time_end = tm.time()
        time_all[epoch - 1] = time_end - time_start
        # test
        if (epoch % args.epochs_test == 0 and epoch>=args.epochs_test_start) or epoch == args.zeroEpoch:
            print("testing of epoch: ", epoch)
            for sample_time in range(1,4):
                ...
                # save graphs
                fname = args.graph_save_path + args.fname_pred(model_index, dataset_index) + str(epoch) +'_'+str(sample_time) + '.dat'
        ...
        if args.save:
            if epoch % args.epochs_save == 0:
                fname = args.model_save_path + args.fname(model_index, dataset_index) + 'lstm_' + str(epoch) + '.dat'
                torch.save(rnn.state_dict(), fname)
                fname = args.model_save_path + args.fname(model_index, dataset_index) + 'output_' + str(epoch) + '.dat'
                torch.save(output.state_dict(), fname)
        epoch += 1
    np.save(args.timing_save_path+args.fname(model_index, dataset_index),time_all)


########### for graph completion task
def train_graph_completion(args, dataset_test, rnn, output, model_index, dataset_index):
    fname = args.model_save_path + args.fname(model_index, dataset_index) + 'lstm_' + str(args.load_epoch) + '.dat'
    rnn.load_state_dict(torch.load(fname))
    fname = args.model_save_path + args.fname(model_index, dataset_index) + 'output_' + str(args.load_epoch) + '.dat'
    output.load_state_dict(torch.load(fname))
    ...
    for sample_time in range(1,4):
        ...
        # save graphs
        fname = args.graph_save_path + args.fname_pred(model_index, dataset_index) + str(epoch) +'_'+str(sample_time) + 'graph_completion.dat'
        save_graph_list(G_pred, fname)
    print('graph completion done, graphs saved')


########### for NLL evaluation
def train_nll(args, dataset_train, dataset_test, rnn, output,graph_validate_len,graph_test_len, model_index, dataset_index, max_iter = 1000):
    fname = args.model_save_path + args.fname(model_index, dataset_index) + 'lstm_' + str(args.load_epoch) + '.dat'
    rnn.load_state_dict(torch.load(fname))
    fname = args.model_save_path + args.fname(model_index, dataset_index) + 'output_' + str(args.load_epoch) + '.dat'
    output.load_state_dict(torch.load(fname))
